package Hibernate;
// Generated 27 mars 2015 15:50:02 by Hibernate Tools 3.6.0



/**
 * PostId generated by hbm2java
 */
public class PostId  implements java.io.Serializable {


     private String iduser;
     private int codedoc;
     private short numberneg;

    public PostId() {
    }

    public PostId(String iduser, int codedoc, short numberneg) {
       this.iduser = iduser;
       this.codedoc = codedoc;
       this.numberneg = numberneg;
    }
   
    public String getIduser() {
        return this.iduser;
    }
    
    public void setIduser(String iduser) {
        this.iduser = iduser;
    }
    public int getCodedoc() {
        return this.codedoc;
    }
    
    public void setCodedoc(int codedoc) {
        this.codedoc = codedoc;
    }
    public short getNumberneg() {
        return this.numberneg;
    }
    
    public void setNumberneg(short numberneg) {
        this.numberneg = numberneg;
    }


   public boolean equals(Object other) {
         if ( (this == other ) ) return true;
		 if ( (other == null ) ) return false;
		 if ( !(other instanceof PostId) ) return false;
		 PostId castOther = ( PostId ) other; 
         
		 return ( (this.getIduser()==castOther.getIduser()) || ( this.getIduser()!=null && castOther.getIduser()!=null && this.getIduser().equals(castOther.getIduser()) ) )
 && (this.getCodedoc()==castOther.getCodedoc())
 && (this.getNumberneg()==castOther.getNumberneg());
   }
   
   public int hashCode() {
         int result = 17;
         
         result = 37 * result + ( getIduser() == null ? 0 : this.getIduser().hashCode() );
         result = 37 * result + this.getCodedoc();
         result = 37 * result + this.getNumberneg();
         return result;
   }   

    @Override
    public String toString() {
        return "PostId{" + "iduser=" + iduser + ", codedoc=" + codedoc + ", numberneg=" + numberneg + '}';
    }


}


